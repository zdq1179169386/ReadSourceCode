...
//通过我们替换的block实例化。也就是将我们传入的block,包装成了AspectIdentifier
@interface AspectIdentifier : NSObject
+ (instancetype)identifierWithSelector:(SEL)selector object:(id)object options:(AspectOptions)options block:(id)block error:(NSError **)error;
- (BOOL)invokeWithInfo:(id<AspectInfo>)info;
@property (nonatomic, assign) SEL selector;
@property (nonatomic, strong) id block;
@property (nonatomic, strong) NSMethodSignature *blockSignature;//block 的签名
@property (nonatomic, weak) id object;//具体信息所属类
...
//    hook 类方法应该去元类中取找
    ...
    return aspect_add((id)catMetal, selector, options, block, error);
}
//入口方法
...
//  AspectIdentifier hook 类的唯一标志，相当于ID
...
// 生成带 _Aspects_ 前缀的子类,然后将当前对象的isa 指向 子类，具体是通过 object_setClass 这个函数，同时修改了 subclass 以及其 subclass metaclass 的 class 方法,使他返回当前对象的 class。
...
//同时修改了 subclass 以及其 subclass metaclass 的 class 方法,使他返回当前对象的 class。
...
//    加前缀
    ...
    // 对象的AspectsContainer,添加到对象的aspect
    AspectsContainer *objectContainer = objc_getAssociatedObject(self, aliasSelector);
    // 类的AspectsContainer，添加类上面的aspect
...
//        hook 的 dealloc 方法，只能在 原始的 dealloc 方法之前hook ，否则报错
        NSString *errorDesc = @"AspectPositionBefore is the only valid position when hooking dealloc.";
        AspectError(AspectErrorSelectorDeallocPosition, errorDesc);
        return NO;
    }
// 对象或者类是否响应传入的selector
